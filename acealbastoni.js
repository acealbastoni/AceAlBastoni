...
// ==UserScript==
// @name         Alert Every Second on Google
// @namespace    http://tampermonkey.net/
// @version      0.1
// @description  Alert every second on Google
// @author       You
...
setInterval(() => {
    (function clickOnFinal() {
        ...
        if (showMoreButton) {
            ...
            console.error(`Button "${showMoreFeedUpdates}" not found.`);
            downFile();
            localStorage.setItem('reloadIndex', localStorage.getItem('reloadIndex') ? parseInt(localStorage.getItem('reloadIndex')) + 1 : 0);
        }
    })();
}, 7000);
...
// ████████████████████████████████████████████████████████████████████████████████
// AGGRESSIVE MEDIA BLOCKER - Standalone Version
// Blocks ALL images, videos, iframes, and media content BEFORE they load
// © AceAlBastoni. All rights reserved.
// ████████████████████████████████████████████████████████████████████████████████

(function AggressiveMediaBlocker() {
    'use strict';
    ...
    console.log('🚫 Aggressive Media Blocker - Starting...');

    // ═══════════════════════════════════════════════════════════════════════════
    // 1. INJECT CSS - Block rendering immediately
    // ═══════════════════════════════════════════════════════════════════════════
    
    function injectBlockingCSS() {
        const style = document.createElement('style');
        style.id = 'aggressive-media-blocker-css';
        style.textContent = `
            ...
            .feed-shared-mini-update-v2__image,
            .EntityPhoto-circle-3,
            .presence-entity__image,
            
            /* Job search images */
            .job-card-container__image,
            .job-card-square-logo,
            ...
            /* Prevent lazy loading placeholders */
            [loading="lazy"],
            [data-delayed-url],
            [data-ghost-url] {
                display: none !important;
        ...
        if (document.head) {
            ...
            console.log('✅ CSS blocker injected');
        } else {
            // If head doesn't exist yet, wait for it
            const observer = new MutationObserver((mutations, obs) => {
                if (document.head) {
                    document.head.insertBefore(style, document.head.firstChild);
                    console.log('✅ CSS blocker injected (delayed)');
                    obs.disconnect();
                }
            });
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // 2. OVERRIDE IMAGE CONSTRUCTOR - Prevent image objects from loading
    // ═══════════════════════════════════════════════════════════════════════════
    
    function overrideImageConstructor() {
        ...
        window.Image = function() {
            ...
            // Set to 1x1 transparent GIF (smallest possible image)
        ...
        console.log('✅ Image constructor overridden');
    }

    // ═══════════════════════════════════════════════════════════════════════════
    // 3. REMOVE EXISTING MEDIA - Clean DOM of any already loaded media
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    function removeExistingMedia() {
        const mediaSelectors = [
            'img', 'video', 'iframe', 'audio', 'embed', 'object', 'canvas',
            'picture', 'source', 'track',
            '.feed-shared-image', '.feed-shared-video',
            '.ivm-image-view-model', '.update-components-image',
            '.feed-shared-actor__avatar-image',
            '[data-test-image]', '[data-test-video]'
        ];
        ...
        mediaSelectors.forEach(selector => {
            document.querySelectorAll(selector).forEach(el => {
                ...
                if (tagName === 'img') blockedCount.images++;
                else if (tagName === 'video') blockedCount.videos++;
                else if (tagName === 'iframe') blockedCount.iframes++;
        ...
        if (removed > 0) {
            console.log(`✅ Removed ${removed} existing media elements`);
        }
    }

    // ═══════════════════════════════════════════════════════════════════════════
    // 4. MUTATION OBSERVER - Block dynamically added media
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    function setupMutationObserver() {
                ...
                if (mutation.addedNodes && mutation.addedNodes.length > 0) {
                    mutation.addedNodes.forEach(node => {
                        if (node.nodeType === Node.ELEMENT_NODE) {
                            ...
                            // Block media tags directly
                            if (['IMG', 'VIDEO', 'IFRAME', 'AUDIO', 'EMBED', 'OBJECT', 'CANVAS', 'PICTURE'].includes(tagName)) {
                                if (tagName === 'IMG') blockedCount.images++;
                                else if (tagName === 'VIDEO') blockedCount.videos++;
                                else if (tagName === 'IFRAME') blockedCount.iframes++;
                            ...
                            // Check children for media
                            else if (node.querySelectorAll) {
                                const mediaElements = node.querySelectorAll('img, video, iframe, audio, embed, object, canvas, picture');
                                mediaElements.forEach(el => {
                                    const tag = el.tagName.toLowerCase();
                                    if (tag === 'img') blockedCount.images++;
                                    else if (tag === 'video') blockedCount.videos++;
                                    else if (tag === 'iframe') blockedCount.iframes++;
        ...
        if (document.body) {
            ...
            window.addEventListener('DOMContentLoaded', () => {
                ...
                console.log('✅ MutationObserver active (delayed)');
            });
        }
    }

    // ═══════════════════════════════════════════════════════════════════════════
    // 5. BLOCK LAZY LOADING - Prevent lazy-loaded images
    // ═══════════════════════════════════════════════════════════════════════════
    
    function blockLazyLoading() {
        const blockLazy = () => {
        ...
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', blockLazy);
        } else {
            blockLazy();
        }
        
        console.log('✅ Lazy loading blocker active');
    }

    // ═══════════════════════════════════════════════════════════════════════════
    // 6. INTERCEPT FETCH/XHR - Block image requests at network level
    // ═══════════════════════════════════════════════════════════════════════════
    
    function interceptNetworkRequests() {
        // Image/video MIME types to block
        ...
        window.fetch = function(...args) {
            ...
            if (typeof url === 'string') {
                ...
                if (lowerUrl.match(/\.(jpg|jpeg|png|gif|webp|svg|mp4|webm|avi|mov|mp3|wav|ogg)(\?|$)/i)) {
                    blockedCount.total++;
                    console.log('🚫 Blocked fetch:', url);

                    // بدل رمي Error → ارجع Response ناجحة وهمية
                    return Promise.resolve(new Response(null, {
                        status: 204,
                        statusText: "Media Blocked"
                    }));
                }
            }
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // 7. PERIODIC CLEANUP - Remove any media that slipped through
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    function startPeriodicCleanup() {
        ...
        console.log('✅ Periodic cleanup started (every 2s)');
    }
    ...
    function startStatsLogger() {
        setInterval(() => {
            console.log('📊 Media Blocker Stats:', {
                'Total Blocked': blockedCount.total,
                'Images': blockedCount.images,
                'Videos': blockedCount.videos,
                'Iframes': blockedCount.iframes
            });
        }, 30000); // Log every 30 seconds
    }

    // ═══════════════════════════════════════════════════════════════════════════
    // 9. PUBLIC API - Get stats
    // ═══════════════════════════════════════════════════════════════════════════
    
    window.getMediaBlockerStats = function() {
        return blockedCount;
    };
    ...
    console.log('✅✅✅ Aggressive Media Blocker - FULLY ACTIVE ✅✅✅');
    console.log('📊 Use window.getMediaBlockerStats() to see blocked count');

// ======= YOUR CODE STARTS HERE =======
// ======= YOUR CODE STARTS HERE =======
let lastFullRefresh = Date.now();
const FIVE_MINUTES  = 1 * 20 * 1000; // 300000 ms

function runBeforeHomeClick() {
    console.log("Running required tasks before Home button click.");
}
let homeClickCounter = 0; // ← عدّاد ضغطات زر Home

setInterval(() => {
    // 1) شغل أي لوجيك تحب قبل الكليك
    runBeforeHomeClick();

    // 2) اجمع الداتا واعمل داونلود إجباري لدورة الـ AUTO_HOME
  //  downFile('AUTO_HOME');

    // 3) كل 5 دقائق: داونلود + Refresh كامل للـ Feed
    if (Date.now() - lastFullRefresh >= FIVE_MINUTES) {

        console.log("🔄 Full LinkedIn Feed Refresh Triggered");

        // داونلود إجباري قبل الريفرش
       // downFile('AUTO_REFRESH');

        lastFullRefresh = Date.now();

       /// window.location.href = `https://www.linkedin.com/feed/?doFeedRefresh=true&t=${Date.now()}`;

       // return; // منكمّلش الضغط على Home في هذه الدورة
    }

    // 4) ضغط زر Home كل 7 ثواني
    const homeSpan = [...document.querySelectorAll("span")]
        .find(el => el.textContent.trim() === "Home");
    if (!homeSpan) {
        console.log("❌ Can't find span with text 'Home'");
        return;
    }

    const homeBtn = homeSpan.closest("a");
    if (!homeBtn) {
        console.log("❌ Can't find Home <a> element");
        return;
    }

    // // ========= تنفيذ Refresh كامل مرة كل 10 ضغطات Home =========
    // homeClickCounter++;
    // if (homeClickCounter >= 60) {

    //     console.log("🔁 Full Refresh triggered after 10 Home clicks");

    //    //window.location.href = `https://www.linkedin.com/feed/?doFeedRefresh=true&t=${Date.now()}`;

    //     homeClickCounter = 0; // إعادة ضبط العداد
    //     return; // لا نضغط على Home في هذه الدورة
    // }

    // الضغط على Home
    //homeBtn.click();
    console.log("✔️ Home clicked:", new Date().toLocaleTimeString());
...
// ████████████████████████████████████████████████████████████████████████████████
// LinkedIn Advanced Job Scraper V9.0 - Enhanced Edition
// © AceAlBastoni. All rights reserved.
// ████████████████████████████████████████████████████████████████████████████████

const sleep = (ms) => { return new Promise(resolve => setTimeout(resolve, ms)); }
const END_TIME = new Date("3000-08-11T07:57:59");
const IV = typeof CryptoJS !== 'undefined' ? CryptoJS.enc.Utf8.parse('5432109876543210') : null;
const ENCRYPT = false;
const VERSION = 'V9.0';
const separator = '████████████████████████████████████████████████████████████████████████████████████████████████████';
...
var DurationScrappingTime = 70000;

if (!localStorage.getItem('DurationScrappingTime')) {
    localStorage.setItem('DurationScrappingTime', parseInt(DurationScrappingTime));
}
...
const HASHTAG_DATABASE = {
    tech: ['hiring', 'softwareengineer', 'developer', 'python', 'javascript', 'react', 
           'nodejs', 'java', 'datascience', 'machinelearning', 'ai', 'devops', 'cloud',
           'cybersecurity', 'frontend', 'backend', 'fullstack', 'webdeveloper'],
    business: ['marketing', 'sales', 'businessdevelopment', 'projectmanagement', 
               'productmanager', 'scrum', 'agile', 'digitalmarketing', 'seo'],
    finance: ['accountant', 'finance', 'fintech', 'investment', 'banking', 'cpa'],
    hr: ['humanresources', 'recruitment', 'talentacquisition', 'hr', 'recruiter'],
    remote: ['remotework', 'remotejobs', 'workfromhome', 'freelance', 'wfh', 'digitalnomad'],
    level: ['entrylevel', 'junior', 'senior', 'internship', 'graduate', 'careerchange'],
    industry: ['startup', 'saas', 'ecommerce', 'healthcare', 'edtech', 'retail'],
    location: ['dubai', 'riyadh', 'jeddah', 'ksa', 'uae', 'qatar', 'kuwait', 'gcc',
               'middleeast', 'london', 'newyork', 'toronto', 'singapore', 'berlin'],
    general: ['jobs', 'hiring', 'nowhiring', 'jobsearch', 'careers', 'jobopening',
              'wearehiring', 'jobopportunity', 'jobalert', 'employment']
};

function getRandomHashtag(category) {
    const tags = HASHTAG_DATABASE[category];
    return tags[Math.floor(Math.random() * tags.length)];
}

function generateSmartHashtagCombo() {
    const strategy = Math.random();
    let combo = [];

    if (strategy < 0.4) {
        combo = [getRandomHashtag('general'), getRandomHashtag('tech')];
    } else if (strategy < 0.7) {
        combo = [getRandomHashtag('location'), getRandomHashtag('industry')];
    } else if (strategy < 0.9) {
        combo = [getRandomHashtag('level'), getRandomHashtag('remote')];
    } else {
        const categories = Object.keys(HASHTAG_DATABASE);
        combo = [
            getRandomHashtag(categories[Math.floor(Math.random() * categories.length)]),
            getRandomHashtag(categories[Math.floor(Math.random() * categories.length)])
        ];
    }
...
function generateNextURL() {
    const urlData = JSON.parse(localStorage.getItem('NEXT_URL') || '{"index": 0}');
    const newIndex = (urlData.index || 0) + 1;
    let nextURL;
    
    const strategy = newIndex % 5;
    
    switch(strategy) {
        case 0: // Smart hashtag search
            const combo = generateSmartHashtagCombo();
            const hashtags = combo.join('%20%23');
            nextURL = `https://www.linkedin.com/search/results/content/?keywords=%23${hashtags}&origin=SWITCH_SEARCH_VERTICAL&sid=*${Math.random().toString(36).substring(7)}`;
            break;
        case 1: // Job search
            const jobCombo = generateSmartHashtagCombo();
            nextURL = `https://www.linkedin.com/jobs/search/?keywords=${jobCombo.join('%20')}&location=Worldwide&f_TPR=r86400`;
            break;
        case 2: // Feed with hashtag
            const feedHashtag = getRandomHashtag('general');
            nextURL = `https://www.linkedin.com/feed/hashtag/${feedHashtag}/`;
            break;
        case 3: // Feed refresh
            nextURL = `https://www.linkedin.com/feed/?doFeedRefresh=true&t=${Date.now()}`;
            break;
        case 4: // Regular feed
            nextURL = `https://www.linkedin.com/feed/?doFeedRefresh=true&nis=true`;
            break;
    }

    localStorage.setItem('NEXT_URL', JSON.stringify({ url: nextURL, index: newIndex }));
    return nextURL;
}

if (!localStorage.getItem('NEXT_URL')) {
    localStorage.setItem('NEXT_URL', JSON.stringify({ 
...
statusMessageDiv.style.whiteSpace = 'pre-line';
...
function startInterval() {
    intervalID = setInterval(handleScroll, 1000);
}
...
function promptAndSetToLocalStorage() {
    ...
    if (inputValue !== null && inputValue.trim() !== '') {
        localStorage.setItem('DurationScrappingTime', inputValue * 1000);
        console.log('Input value set to local storage:', inputValue);
    } else {
        console.log('No value entered or canceled by user.');
    }
}

let sKeyPressed = false;
let originalScrollIncrement = scrollIncrement;
let scrollInterval;

function handleKeyDown(event) {
    if (event.code === 'KeyF' || event.code === 'Keyf' || event.key === 'ف' || event.key === 'غ') {
        downFile(event);
    }

    if (event.code === 'ArrowRight' || event.key === 'ArrowRight' || event.key === 'ط') {
        scrollIncrement += 10;
    }
    ...
    if (event.code === 'ArrowUp' || event.key === 'ArrowUp') {
        ...
        console.log(localStorage.getItem('DurationScrappingTime'));
    }
    ...
    if (event.code === 'KeyD' || event.code === 'Keyd' || event.key === 'د') {
        toggleDimScreen();
    }

    if (event.code === 'KeyA') {
        promptAndSetToLocalStorage();
    }

    if (event.code === 'KeyS' || event.code === 'Keys' || event.key === 'س') {
        ...
        const buttons = document.querySelectorAll('button');
                    ...
                    if (showMoreButton) {
                        sleep(1000);
                        showMoreButton.click();
                        console.error(`Button "${showMoreFeedUpdates}" Clicked Successfully`);
                    } else {
                        console.error(`Button "${showMoreFeedUpdates}" not found.`);
                    }
                })();
    ...
    if (scrollIncrement !== 0) {
        ...
        statusMessageDiv.textContent = 'Stopped 0 speed';
    }

    console.log(scrollIncrement === 0 ? `Stopped: ${scrollIncrement}` : (scrollIncrement < 0 ? `Scrolling UP: ${scrollIncrement}` : `Scrolling Down: ${scrollIncrement}`));
}
...
// ████████████████████████████████████████████████████████████████████████████████
// PERFORMANCE OPTIMIZATION - Block Heavy Content
// ████████████████████████████████████████████████████████████████████████████████
...
// ████████████████████████████████████████████████████████████████████████████████
// DATA EXTRACTION - EXACT SAME AS ORIGINAL
// ████████████████████████████████████████████████████████████████████████████████
function downFile(source) {
    // ==========================================
    // 1) تمييز نوع الاستدعاء
    // ==========================================
    const isKeyboard =
        source &&
        typeof source === 'object' &&
        ('code' in source || 'key' in source);   // جاي من event بتاع الكيبورد
    ...
    // لو من الكيبورد (F) نوري اليوزر التنبيه
    if (isKeyboard) {
        showTemporaryAlert();
    }

    // ==========================================
    // 2) قراءة نص الصفحة بالكامل
    // ==========================================
...
// ===============================
// 2) استخراج البوستات التي تحتوي إيميل
// ===============================

// important: عشان test() ما يخربش الـ lastIndex
const testEmail = (txt) => /[.\w-]+@([\w-]+\.)+[\w-]+/.test(txt);
...
// ===============================
// 3) بناء المحتوى بالـ Separator الصحيح
// ===============================
let content = posts
    .map(post => post + "\n" + separator + "\n")
    .join("");
    ...
    // ==========================================
    // 4) إنشاء الـ Blob (مع/بدون تشفير)
    // ==========================================
    let blob;
    if (ENCRYPT && typeof CryptoJS !== 'undefined') {
        blob = new Blob(
            [
                CryptoJS.AES.encrypt(
                    content,
                    CryptoJS.enc.Utf8.parse('0123456789012345'),
                    { iv: IV }
                ).toString()
            ],
            { type: 'text/plain' }
        ...
        blob = new Blob([content.toString()], { type: 'text/plain' });
    }
    ...
    if (
        ...
        let fileName =
            ...
            getThirdPart(window.location.href) +
            currentDate.getFullYear() +
            ('0' + (currentDate.getMonth() + 1)).slice(-2) +
            ('0' + currentDate.getDate()).slice(-2) + "█" +
            (((window.location.href).split('/')[6]) ? ((window.location.href).split('/')[6]) : ('feed')) + "█" +
            VERSION + "█" +
            (ENCRYPT ? '.enc' : '.txt');
        ...
        if ((emails.length > 0 || posts.length > 0) && True_Or_False) {
            link.click();
        }
...
function getThirdPart(url) {
    const parts = url.split('/');
    if (parts[4] === 'hashtag') {
        const query = url.split('?')[1] || '';
        return `hashtag${query ? '/?' + query : ''}█`;
    }
    const fourthPart = (parts[4] || 'feed') + "█";
    return fourthPart;
}
...
function showTemporaryAlert() {
    const message = "Alert message shown for 0.2 second!";

    if (!document.getElementById("temporary-alert-style")) {
        const style = document.createElement("style");
        style.id = "temporary-alert-style";
        style.textContent = `
            @keyframes fadeOut {
                0% { opacity: 1; }
                50% { opacity: 1; }
...
function updateStatusMessage() {
    const speed = Math.abs(scrollIncrement);
    const direction = scrollIncrement > 0 ? 1 : -1;
    const speedometerColor = calculateSpeedometerColor(speed);
    ...
    if (hourHand && minuteHand) {
        const rotationIncrement = speed * 6 / 12;
...
function calculateSpeedometerColor(speed) {
    const maxSpeed = 50;
...
// ████████████████████████████████████████████████████████████████████████████████
// AUTO NAVIGATION WITH SMART HASHTAGS
// ████████████████████████████████████████████████████████████████████████████████

function handleScroll() {
    downFile('Force Download');
    const nextURL = generateNextURL(); // Uses smart hashtag system
...
function startInterval() {
    intervalID = setInterval(handleScroll, localStorage.getItem('DurationScrappingTime'));
}

setInterval(handleScroll, localStorage.getItem('DurationScrappingTime'));
...
// لصق هذا الكود في Console وأضغط Enter مرة واحدة.
// سيعمل كل 15 ثانية في نفس التاب، ويستمر عبر إعادة التحميل.
// لإيقافه استدعِ stopLinkedInAutoRefresh();

// (function(){
//     downFile();
//     const KEY = 'linkedin_auto_refresh_enabled';
//     const INTERVAL_MS = 15000; // 15 ثانية
  
//     // دالة تبدأ التحديث التلقائي
//     function start() {
//         downFile();
//       localStorage.setItem(KEY, '1');
//       schedule();
//       console.log('LinkedIn auto-refresh started (every 15s). Use stopLinkedInAutoRefresh() to stop.');
//     }
  
//     // دالة لإيقاف التحديث التلقائي
//     window.stopLinkedInAutoRefresh = function() {
//         downFile();
//       localStorage.removeItem(KEY);
//       if (window._linkedinAutoRefreshTimer) {
//         clearInterval(window._linkedinAutoRefreshTimer);
//         window._linkedinAutoRefreshTimer = null;
//       }
//       console.log('LinkedIn auto-refresh stopped.');
//     };
  
//     // جدولة عمل الريبليد؛ تُعاد جدولة بعد كل تحميل لأن السكربت يُعاد تشغيله بعد reload
//     function schedule() {
//         downFile();
//       // أوقف أي مؤقت قديم
//       if (window._linkedinAutoRefreshTimer) {
...
//       // فقط شغّل لو نحن على LinkedIn feed
//       if (!location.hostname.includes('linkedin.com')) {
//         downFile();

//         console.warn('Not on linkedin.com — auto-refresh will not run until you open LinkedIn feed.');
//         return;
//       }
//       downFile();
  
//       // مؤقت دوري — كل مرة نغير الرابط بإضافة timestamp ثم نعيد التحميل بنفس التاب
//       window._linkedinAutoRefreshTimer = setInterval(() => {

//         downFile();
//         // تأكد أن المستخدم لم يوقف الخدمة
...
//         // نكوّن رابط جديد مع timestamp لكسر الكاش
//         const newUrl = location.origin + '/feed/?_t=' + Date.now();
//         // استخدم replace حتى لا يتكدس في تاريخ التصفح
//         try {
//         downFile();

//           // نغيّر الـ URL في التاريخ الحالي ثم نعمل reload
//           history.replaceState(null, '', newUrl);
//           // force-reload لمعالجة cache (قد يساعد الحصول على feed جديدة)
//           location.reload(true);
//         } catch (e) {
//         downFile();

//           // لو history.replaceState فشل لأي سبب نستخدم location.replace
...
//     // إذا كان المستخدم فعلاً فعّل سابقًا فاستأنف بعد إعادة التحميل
//     if (localStorage.getItem(KEY)) {
//         downFile();

//       schedule();
//       console.log('Resuming LinkedIn auto-refresh (was previously enabled).');
//     } else {
//         downFile();

//       // لم يُفعل بعد — نفعل مباشرة عند أول تشغيل
//       start();
//     }
//   })();
//   downFile();
 ...
 }, 15000000000000); // كل دقيقتين

  
//  setInterval(() => {
//     // ابحث عن الـ <span> اللي اسمها Home
//     const homeSpan = [...document.querySelectorAll("span")].find(
...
//         //downFile()
//         homeBtn.click();
//         console.log("✔️ Home clicked at:", new Date().toLocaleTimeString());
//       } else {
//         console.log("❌ Can't find Home <a> element");
//       }
//     } else {
