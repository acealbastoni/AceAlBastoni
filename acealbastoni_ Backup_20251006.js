...
const END_TIME = new Date("3000-08-11T07:57:59"); // Format: "YYYY-MM-DDTHH:MM:SS"
const IV = CryptoJS.enc.Utf8.parse('5432109876543210');
const ENCRYPT = false;
const VERSION = 'V8.2'
const separator = '████████████████████████████████████████████████████████████████████████████████████████████████████';
var True_Or_False = true;
let previousSeparatorCount = 0; // Variable to store the previous separator count
...
// Create a div for displaying the status message
...
statusMessageDiv.style.zIndex = '9999'; // Ensure it's above other elements
...
// Function to create and append the dimming overlay
...
// Function to toggle dimming overlay visibility
...
function startInterval() {
    intervalID = setInterval(handleScroll, 1000); // Set interval for every 1000 milliseconds (1 second)
}
...
function promptAndSetToLocalStorage() {
    ...
    if (inputValue !== null && inputValue.trim() !== '') {
        // Set the input value to local storage
...
let originalScrollIncrement = scrollIncrement; // Set your original scroll increment value here
let scrollInterval;
function handleKeyDown(event) {
    // Check if the key pressed is the F or f
    // if (event.key === 'f' || event.key === 'F') {
    //     downFile(event); // force Download file
    // }
    ...
    if (event.code === 'ArrowRight' || event.key === 'ArrowRight' || event.key === 'ط') {
        scrollIncrement += 10; // Increase the scroll increment by 10
    }
    ...
    if (event.code === 'ArrowUp' || event.key === 'ArrowUp') {
        localStorage.setItem('DurationScrappingTime',parseInt(localStorage.getItem('DurationScrappingTime')) + (1000))
         // Increase the scroll increment by 10
    ...
    // Check if the key pressed is 'D' to toggle dimming screen
    if (event.code === 'KeyD' || event.code === 'Keyd' || event.key === 'د') {
        toggleDimScreen();
    }


    // Check if the key pressed is 'A' to Update The Durlation time for a link
    if (event.code === 'KeyA') {
    ...
    // Check if the key pressed is 's' to toggle scrollIncrement
    if (event.code === 'KeyS' || event.code === 'Keys' || event.key === 'س') {
        sKeyPressed = !sKeyPressed; // Toggle the state
        ...
        if (sKeyPressed) {
            ...
            scrollInterval = setInterval(()=>{
                ...
                // Find the button using the custom function
    ...
    if (scrollIncrement !== 0) {
        ...
        // Clear the message when scrolling stops
...
//let lastSeparatorCount = 0; // Variable to store the last separatorCount value
// Declare separatorCount variable
...
function showTemporaryAlert() {
    ...
    // Create a style tag for animation (only once)
...
function downFile(keyF_Pressed) {
    ...
    let posts = [
        ...
        .forEach((el) => postsSet.add(el.innerText));
    ...
    if (
        ...
        previousSeparatorCount = separatorCount; // Update the previous separator count
    }
...
function getThirdPart(url) {
    ...
    // Check if the fourth part is 'hashtag'
    if (parts[4] === 'hashtag') {
    ...
    // Use the fourth part or 'feed' as the default
...
//██████████████████████████████████████████████████████████████████████████████████████████████████████████████████
// Function to update the status message with separatorCount value
function updateStatusMessage() {
    const speed = Math.abs(scrollIncrement);
    const direction = scrollIncrement > 0 ? 1 : -1;
    // Calculate speedometer color based on speed value
    const speedometerColor = calculateSpeedometerColor(speed);
    // Update status message with moving clock-style speedometer design
    ...
    if (hourHand && minuteHand) {
        const rotationIncrement = speed * 6 / 12; // Adjust as needed
        ...
        // Automatically move the hour hand in a circular path
        setInterval(() => {
            const currentTime = new Date();
            const seconds = currentTime.getSeconds();
            const degrees = (seconds * 6) % 360; // 6 degrees per second
            hourHand.style.transform = `rotate(${degrees}deg)`;
        }, 1000); // Update every second
    }
}
// Function to calculate speedometer color based on speed value
function calculateSpeedometerColor(speed) {
    // Example: Use a gradient from green (low speed) to red (high speed)
    const maxSpeed = 50; // Adjust as needed
...
setInterval(() => {
    downFile('Force Download');
    // Find the element with the class name 'scaffold-finite-scroll__content'
...
function handleScroll() {
    ...
    // window.location.replace("https://www.linkedin.com/feed/?cache=" + new Date().getTime());
...
// Function to start the interval
function startInterval() {
    intervalID = setInterval(handleScroll, localStorage.getItem('DurationScrappingTime')); // Set interval for every 1000 milliseconds (1 second)
}
...
if (showMoreButton) {
  ...
  console.error(`Button "${showMoreFeedUpdates}" not found.`);
     //window.location.href = 'https://www.linkedin.com/feed/?doFeedRefresh=true&nis=false';
     downFile();
