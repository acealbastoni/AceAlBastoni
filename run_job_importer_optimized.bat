...
REM ⚡ OPTIMIZED: Use wildcard to filter files directly (much faster!)
for /f "delims=" %%F in ('dir /B *█*.txt 2^>nul') do (
    
    REM ⚡ OPTIMIZED: Direct string manipulation without pipe/findstr
    set "filename=%%~nF"
    ...
    if not "!target_folder!"=="" (
        ...
        REM ⚡ OPTIMIZED: Direct move without lock check (faster)
